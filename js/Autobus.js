define("Autobus", function () {
    ...
    Router.prototype = {
         ...
         */
        addRoute: function (start, goal, onChange) {
            this.routeList.push({
                start: start, 
                goal: goal,
                onChange: onChange
         ...
         * 
         * Paramter:
         *      obstacle {left, top, right, bottom} - A rectangle to avoid
         */
        addObstacle: function (obstacle) {
            this.obstacleList.push(rect);
            this.extendLimits(rect);

            // Compute the cost of passing through an obstacle
            var width  = this.limitsRect.right  - this.limitsRect.left;
            var height = this.limitsRect.bottom - this.limitsRect.top;
         ...
         * the given rectangle.
         */
        extendLimits: function (aRect) {
            if (aRect.left < this.limitsRect.left + this.options.margin) {
                this.limitsRect.left = aRect.left - this.options.gridStep;
                this.allocate = true;
            }
            if (aRect.right > this.limitsRect.right - this.options.margin) {
                this.limitsRect.right = aRect.right + this.options.margin;
                this.allocate = true;
            }
            if (aRect.top < this.limitsRect.top + this.options.margin) {
                this.limitsRect.top = aRect.top - this.options.margin;
                this.allocate = true;
            }
            if (aRect.bottom > this.limitsRect.bottom - this.options.margin) {
                this.limitsRect.bottom = aRect.bottom + this.options.margin;
                this.allocate = true;
            }
        },
        ...
        route: function () {            
            ...
            function propagateGroupId(refRoute, refRouteIndex) {
                for (var routeIndex = refRouteIndex + 1; routeIndex < self.routeList.length; routeIndex ++) {
                    var route = self.routeList[routeIndex];
                    if (!("groupId" in route) &&
         ...
         */
        initGrid: function () {
            var gridColumns = Math.floor((this.limitsRect.right - this.limitsRect.left) / this.options.gridStep) + 1;
            var gridRows    = Math.floor((this.limitsRect.bottom - this.limitsRect.top) / this.options.gridStep) + 1;
            
            if (this.allocate) {
                this.grid = new Array(gridColumns);
            }
            
            for (var col = 0, x = this.limitsRect.left; col < gridColumns; col ++, x += this.options.gridStep) {
                if (this.allocate) {
                    this.grid[col] = new Array(gridRows);
                }
                
                for (var row = 0, y = this.limitsRect.top; row < gridRows; row ++, y += this.options.gridStep) {
                    // If the current grid node is inside an obstacle,
                    // set the "obstacle" property of the node.
                    var obstacle = this.obstacleList.some(function (rect) {
        ...
        findPath: function (route) {
            ...
            var startCol = Math.round((route.start.x - this.limitsRect.left) / this.options.gridStep);
            var startRow = Math.round((route.start.y - this.limitsRect.top)  / this.options.gridStep);
            route.start.node = this.grid[startCol][startRow];

            var goalCol = Math.round((route.goal.x - this.limitsRect.left) / this.options.gridStep);
            var goalRow = Math.round((route.goal.y - this.limitsRect.top)  / this.options.gridStep);
            ...
            while (openSet.size()) {
                ...
                neighbours.forEach(function (neighbourNode) {
                    ...
                    var g = currentNode.g + this.options.distance(currentNode, neighbourNode);
                    ...
                    var otherNeighbours = this.getNeighbours(neighbourNode);
            ...
            for (var i = 1; i < pathData.length - 1;) {
