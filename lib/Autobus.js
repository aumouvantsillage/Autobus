...
export class Router {
    ...
    route() {
        ...
        const dist = self.options.distance;
        ...
        function propagateGroupId(refRoute, refRouteIndex) {
            for (let routeIndex = refRouteIndex + 1; routeIndex < self.routes.length; routeIndex ++) {
                const route = self.routes[routeIndex];
                if (!("groupId" in route) &&
    ...
    findPath(route) {
        ...
        while (openSet.size) {
            ...
            // Candidate neighbors are compatible with the heading of the current node.
