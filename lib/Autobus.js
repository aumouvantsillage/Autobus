...
export class Router {
     ...
     * When the coordinates are modified in these objects, re-routing will take
     * the modifications into account.
     * You can also define "x" and "y" as getters if the coordinates depend on other objects.
     */
    addRoute(start, goal, onChange) {
        const route = {start, goal, onChange, groupId: this.routes.length};

        // Find whether the new route belongs to an existing group.
        // FIXME groupId is not updated when connections are changed.
        for (const r of this.routes) {
            if (samePoint(route.start, r.start) ||
                samePoint(route.start, r.goal)  ||
                samePoint(route.goal,  r.start) ||
                samePoint(route.goal,  r.goal))
            {
                route.groupId = r.groupId;
                break;
            }
        }
    ...
    findPath(route) {
        ...
        const startNode = this.grid[startCol][startRow];
        ...
        const goalNode  = this.grid[goalCol][goalRow];

        // Assign the f and g scores of the start node and add it to the open set
        startNode.g    = 0;
        startNode.f    = this.options.distance(startNode, goalNode);
        startNode.open = true;
        ...
        let currentNode;
        while (openSet.size) {
            // Get node with lowest f score from the open set.
            ...
            for (const n of this.getNeighbors(currentNode)) {
                // Compute the g score of n starting from the current node
                ...
                else if (!n.groups[route.groupId]) {
                ...
                if (!n.visited || g < n.g) {
                    ...
                    n.g = g;
                    n.f = g + this.options.distance(n, goalNode);
